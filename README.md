...
このプロジェクトには以下の2つの主要なスケッチが含まれています：

1. **`ESP32.ino` (本番コード)**
   - トロッコに実際に書き込んで使用する、Wi-Fi (UDP) 通信対応のメインファームウェアです。
2. **`test/test.ino` (テストコード)**
   - 回路が正しく組めているか（モーターとドライバの配線など）をテストするための単体確認用コードです。Wi-Fi を使わず、ハードウェアの動作確認のみを行います。

### テストスケッチの動作 (`test/test.ino`)
